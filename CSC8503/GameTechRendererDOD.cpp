...
void RendererSystemDOD::Destroy() {
	if (resources.defaultShader) { delete resources.defaultShader; resources.defaultShader = nullptr; }
	if (resources.shadowShader) { delete resources.shadowShader; resources.shadowShader = nullptr; }
	if (resources.skyboxShader) { delete resources.skyboxShader; resources.skyboxShader = nullptr; }
	if (resources.debugShader) { delete resources.debugShader; resources.debugShader = nullptr; }

	if (resources.skyboxMesh) { delete resources.skyboxMesh; resources.skyboxMesh = nullptr; }

	if (resources.textTexVBO != 0) { glDeleteBuffers(1, &resources.textTexVBO); resources.textTexVBO = 0; }
	if (resources.textColourVBO != 0) { glDeleteBuffers(1, &resources.textColourVBO); resources.textColourVBO = 0; }
	if (resources.textVertVBO != 0) { glDeleteBuffers(1, &resources.textVertVBO); resources.textVertVBO = 0; }
	if (resources.textVAO != 0) { glDeleteVertexArrays(1, &resources.textVAO); resources.textVAO = 0; }

	if (resources.shadowTex != 0) { glDeleteTextures(1, &resources.shadowTex); resources.shadowTex = 0; }
	if (resources.skyboxTex != 0) { glDeleteTextures(1, &resources.skyboxTex); resources.skyboxTex = 0; }
	if (resources.shadowFBO != 0) { glDeleteFramebuffers(1, &resources.shadowFBO); resources.shadowFBO = 0; }
